...
export class Settings extends BaseEntity {
	...
	static getSettingsValues(settings: Settings): SettingsValues {
		return {
			...
			bazarr: {
				baseUrl: settings.bazarrBaseUrl || '',
				apiKey: settings.bazarrApiKey || ''
			},
			stremio: {
				addonUrl: settings.stremioAddonUrl || ''
			},
			initialised: true
		};
	}
	...
	public static async set(name: string, values: SettingsValues): Promise<Settings | null> {
		...
		settings.tmdbApiKey = values.tmdb?.apiKey ?? '';
		settings.bazarrBaseUrl = values.bazarr?.baseUrl ?? '';
		settings.bazarrApiKey = values.bazarr?.apiKey ?? '';
		settings.stremioAddonUrl = values.stremio?.addonUrl ?? '';
