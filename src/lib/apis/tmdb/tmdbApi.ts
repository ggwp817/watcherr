...
function getTmdbKey(): string {
	...
	return s.tmdb?.apiKey || (import.meta.env.VITE_TMDB_API_KEY as string | undefined) || '';
}
...
let _tmdbClient: ReturnType<typeof createClient<paths>> | null = null;
let _tmdbClientKey = '';

function getTmdbClient() {
	const key = getTmdbKey();
	if (!_tmdbClient || key !== _tmdbClientKey) {
		_tmdbClientKey = key;
		_tmdbClient = createClient<paths>({
...
export const TmdbApiOpen = new Proxy({} as ReturnType<typeof createClient<paths>>, {
	get(_target, prop) {
