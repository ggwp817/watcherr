...
export const addMovieToRadarr = async (tmdbId: number, qualityProfileIdOverride?: number) => {
	...
	const options: RadarrMovieOptions = {
		qualityProfileId: qualityProfileIdOverride ?? (get(settings)?.radarr.qualityProfileId || 0),
 ...
 * `getRadarrQualityProfiles(baseUrl, apiKey)` axios helper used by the settings page.
 */
export const listRadarrQualityProfiles = async (): Promise<RadarrQualityProfile[]> => {
	const res = await getRadarrApi()?.get('/api/v3/qualityprofile', { params: {} });
	const data = (res?.data ?? []) as any[];
	return data.map((p) => ({ id: p.id, name: p.name }));
};

export const triggerRadarrMovieSearch = async (movieId: number): Promise<boolean> => {
	const res = await getRadarrApi()?.post('/api/v3/command', {
		params: {},
		body: { name: 'MoviesSearch', movieIds: [movieId] } as any
	});
	return !!res?.response.ok;
};

/**
 * One-shot request: add movie (if new) with the given quality profile and trigger a search.
 * If the movie already exists in Radarr, updates its quality profile and kicks off a fresh search.
 */
export const requestMovie = async (tmdbId: number, qualityProfileId: number) => {
	const existing = await getRadarrMovieByTmdbId(String(tmdbId));
	if (existing?.id) {
		// Update profile if changed, then search.
		if (existing.qualityProfileId !== qualityProfileId) {
			await getRadarrApi()?.put('/api/v3/movie/{id}', {
				params: { path: { id: String(existing.id) } },
				body: { ...existing, qualityProfileId } as any
			});
		}
		await triggerRadarrMovieSearch(existing.id);
		return existing;
	}
	// Fresh add: addMovieToRadarr already supports searchForMovie via settings; we force it on.
	const tmdbMovie = await getTmdbMovie(tmdbId);
	...
	const added = await getRadarrApi()
		?.post('/api/v3/movie', { params: {}, body: options })
		.then((r) => r.data);
	return added;
};