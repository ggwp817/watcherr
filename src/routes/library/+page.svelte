<script lang="ts">
	...
	import DownloadingCard from '$lib/components/DownloadingCard/DownloadingCard.svelte';
	...
	let downloadProps: ComponentProps<DownloadingCard>[] = [];
	$: {
		const sonarrProps: ComponentProps<DownloadingCard>[] =
			$servarrDownloadsStore.sonarrDownloads?.map((item) => {
				...
				return {
					...
					backdropUrl: poster?.remoteUrl || poster?.url || ''
				};
			}) || [];

		const radarrProps: ComponentProps<DownloadingCard>[] =
